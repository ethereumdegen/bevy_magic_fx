...
use crate::{magic_fx_variant::{MagicFxInstance, MagicFxVariant}, util::lerp_transforms};
...
pub fn update_magic_fx_instances ( 
		mut magic_fx_query: Query<(Entity,&mut Visibility, &mut Transform, &MagicFxInstanceComponent)>,
		time: Res<Time>
	){
	...
	for (entity,mut fx_visibility, mut fx_xform, instance_comp) in magic_fx_query.iter_mut(){
		...
		*fx_visibility = match is_visible {
		 ...
		 if is_visible {
            ...
            let lerp_amount:f32 = if duration > 0.0 { elapsed / duration } else { 0.0 }.clamp(0.0, 1.0);
